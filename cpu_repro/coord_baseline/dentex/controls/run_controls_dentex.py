...
def features_for_condition(condition, train_df, test_df, shuffled_train_df, shuffled_test_df):
    ...
    if condition == "full (shuffled per image)":
        return shuffled_train_df[FEATURE_COLS].to_numpy(), shuffled_test_df[FEATURE_COLS].to_numpy()
...
def main():
    ...
    df = load_instances()
    print(f"Loaded {len(df)} DENTEX instances from {df['image_id'].nunique()} images")
    ...
    for seed in SEEDS:
        ...
        for condition in CONDITIONS:
            x_train, x_test = features_for_condition(condition, train_df, test_df, shuffled_train_df, shuffled_test_df)
        ...
        print(f"seed {seed} done (train_images={train_df['image_id'].nunique()} test_images={test_df['image_id'].nunique()})")
    ...
    for i, clf_name in enumerate(classifiers):
        ...
        ax.bar(x + i * width, sub["top1_acc_mean"], width, yerr=sub["top1_acc_ci95"], label=clf_name, capsize=4)
    ...
    ax.set_title("DENTEX coordinate-signal ablation")
    ax.legend()
    ax.set_ylim(0, 1)
    fig.tight_layout()
