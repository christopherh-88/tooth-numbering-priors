"""Seed 8 runner for YOLOv8x on local Apple Silicon (MPS). Same image split
as the Kaggle CUDA runner train_yolo_seed8.py (image_split_seed8.csv), so the
...
train_yolo.py; differs from the CUDA runner only in:
  ...
  - separate run/eval directories (yolov8_mps_seed8split,
    eval_results/yolo_mps/seed8) so it never overwrites CUDA results.

Pause/resume: kill and rerun freely; train_yolo.py resumes from
runs/yolov8_mps_seed8split/weights/last.pt.

Usage (local): python train_yolo_mps_seed8.py
"""
