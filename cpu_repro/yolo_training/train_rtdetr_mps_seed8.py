"""Seed 8 runner for RT-DETR-l on local Apple Silicon (MPS). Same image split
as the Kaggle CUDA runner train_rtdetr_seed8.py (image_split_seed8.csv). Thin
override wrapper around train_rtdetr.py; differs from the CUDA runner only in:
  ...
  - separate run/eval directories (rtdetr_mps_seed8split,
    eval_results/rtdetr_mps/seed8) so it never overwrites CUDA results.

Pause/resume: kill and rerun freely; train_rtdetr.py resumes from
runs/rtdetr_mps_seed8split/weights/last.pt.

Usage (local): python train_rtdetr_mps_seed8.py
"""
