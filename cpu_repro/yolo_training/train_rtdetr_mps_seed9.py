"""Seed 9 runner for RT-DETR-l on local Apple Silicon (MPS). Same image split
as the Kaggle CUDA runner train_rtdetr_seed9.py (image_split_seed9.csv). Thin
override wrapper around train_rtdetr.py; differs from the CUDA runner only in:
  ...
  - separate run/eval directories (rtdetr_mps_seed9split,
    eval_results/rtdetr_mps/seed9) so it never overwrites CUDA results.

Pause/resume: kill and rerun freely; train_rtdetr.py resumes from
runs/rtdetr_mps_seed9split/weights/last.pt.

Usage (local): python train_rtdetr_mps_seed9.py
"""
