"""Seed 7 runner for RT-DETR-l on local Apple Silicon (MPS). Same image split
as the Kaggle CUDA runner train_rtdetr_seed7.py (image_split_seed7.csv). Thin
override wrapper around train_rtdetr.py; differs from the CUDA runner only in:
  ...
  - separate run/eval directories (rtdetr_mps_seed7split,
    eval_results/rtdetr_mps/seed7) so it never overwrites CUDA results.

Pause/resume: kill and rerun freely; train_rtdetr.py resumes from
runs/rtdetr_mps_seed7split/weights/last.pt.

Usage (local): python train_rtdetr_mps_seed7.py
"""
